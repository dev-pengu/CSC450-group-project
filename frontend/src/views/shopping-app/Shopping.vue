<template>
  <div class="shopping">
    <v-data-iterator
      ...
      :sort-by="sortBy"
      :sort-desc="sortDesc"
      no-data-text="You don't have any shopping lists. Create one for your family!"
      hide-default-footer
    >
      <template #header>
        ...
        <v-row>
          <v-col v-for="(list, i) in props.items" :key="i" cols="12" sm="6" md="4" lg="3">
            <v-hover v-slot="{ hover }">
              <v-card :elevation="hover ? 8 : 2" :to="{ path: '/shopping/list', query: { id: list.id } }">
                <v-card-title
                ...
                <v-list dense>
                  ...
                  <v-list-item>
                    <v-list-item-content :class="{ 'foa_button--text': sortBy === 'items' }">
                      Items:
                    </v-list-item-content>
                    <v-list-item-content class="align-end" :class="{ 'foa_button--text': sortBy === 'items' }">
                      {{ list.items.length }}
                    </v-list-item-content>
                  </v-list-item>
                </v-list>
...
export default {
  ...
  data: () => ({
    keys: [
      ...
      { name: 'items', friendlyName: 'Number of items' },
    ],
    itemsPerPageArray: [4, 8, 12],
    search: '',
    searchFilters: { FAMILY: [], SHOPPING_LIST: [] },
    itemsPerPage: 8,
    page: 1,
    sortBy: 'created',
    sortDesc: true,
  ...
  computed: {
    ...
    numberOfPages() {
      return Math.ceil(this.shoppingLists.length / this.itemsPerPage);
    },
    formTitle() {
  ...
  methods: {
    ...mapActions(['showSnackbar', 'fetchFamilies']),
    nextPage() {
    ...
    editItem(item) {
      this.editedIndex = this.shoppingLists.indexOf(item);
      this.editedItem = { ...item };
    ...
    async confirmDeleteList() {
      try {
        const res = await api.deleteShoppingList(this.editedItem.id);
        if (res.status === 200) {
          this.closeDelete();
          this.fetchShoppingLists();
          this.showSnackbar({
    ...
    async fetchShoppingLists() {
      try {
        ...
        const res = await api.searchShoppingLists(request);
        if (res.status === 200) {
          this.shoppingLists = res.data.lists;
        } else {
          this.showSnackbar({
            type: 'error',
        ...
        this.searchFilters.SHOPPING_LIST = [];
      }
    },
