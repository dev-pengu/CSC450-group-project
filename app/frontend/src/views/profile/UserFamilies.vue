    ...
    <v-row>
      ...
      <v-col cols="12" sm="7" md="8" lg="9">
        ...
        <v-sheet max-width="400" color="#00000000">
          ...
          <div class="d-flex">
            <h4 class="text-h5 foa_text_header--text my-5">Manage Families</h4>
            <v-btn class="mt-4 ml-4" icon :disabled="loading" :loading="loading" @click="fetchFamilies"
              ><v-icon>mdi-cached</v-icon>
            </v-btn>
          </div>
          <v-card v-for="(family, i) in families" :key="family.id" class="mb-2">
            <v-card-title>
              {{ family.name }}
              <v-spacer></v-spacer>
              <v-btn
                v-if="(!showUpdateForm && isAdmin(family.memberData.role)) || updateIndex !== i"
                icon
                fab
                @click="expand(i)"
              >
              ...
              <v-btn v-else-if="showUpdateForm && updateIndex === i" icon fab @click="collapse">
            ...
            <v-row v-if="updateIndex === i && showUpdateForm" justify="center">
              <v-col cols="9">
            ...
            <v-card-actions class="pb-6">
              <v-row v-if="(showUpdateForm && updateIndex !== i) || !showUpdateForm" justify="center">
                <v-col cols="6">
                  <v-btn
                  ...
                  <v-btn
                    ...
                    :disabled="loading"
                    small
                    @click="leaveFamily(family.id)"
                  >
                    Leave Family
                  </v-btn>
                </v-col>
                <v-col v-if="isAdmin(family.memberData.role)" cols="4">
                  <v-btn
                    class="foa_button_text--text"
                    color="foa_button"
                    elevation="2"
...
export default {
  ...
  methods: {
    ...
    leaveFamily(id) {
      console.log(id);
      // TODO add api call to leave a family after backend endpoint has been made
    },
  },
};
</script>
