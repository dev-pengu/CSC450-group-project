...
public class FamilyRepositoryImpl extends QuerydslRepositorySupport
  ...
  public List<Family> getFamiliesByUserId(Long userId) {
    List<Family> families = from(familyTable).innerJoin(familyTable.members, familyMembersTable)
  ...
  public List<Long> getFamilyIdsByUser(String username) {
    List<Long> ids = from(familyTable).innerJoin(familyTable.members, familyMembersTable)
        .select(familyTable.id)
        .distinct()
        .where(familyMembersTable.user.username.eq(username))
        .fetch();
    return ids;
  }
